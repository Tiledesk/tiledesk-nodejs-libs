...
describe('CHATBOT: Play a prompt', async () => {
    before(function(){
        this.timeout(10000);
        return new Promise(async (resolve, reject) => {
            ...
            assert(configHtml);
            console.log('connected to dep:',DEP_ID)
    ...
    it('Catch a prompt (~1s)', () => {
        return new Promise(async(resolve, reject)=> {
            ...
            async function repeat(count){
                ...
                console.log("STEP 2: time to get a prompt response from start time (/nextblock):", diffPromptResponseTime, '[ms]')
            
                let diffWaitTimeResponseTime = endTimePromtResponse - startTimeFirstWait
                console.log("STEP 3: time to get a prompt response after first wait time (/nextblock has a message in queue):", diffWaitTimeResponseTime, '[ms]')
