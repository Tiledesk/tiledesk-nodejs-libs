    ...
    it('Catch a SPEECH form (~1s)', () => {
        return new Promise(async(resolve, reject)=> {
            ...
            async function repeat(count){
                ...
                console.log("STEP 2: time to get a speech action from start time (/nextblock):", diffPromptResponseTime, '[ms]')

                let diffWaitTimeResponseTime = endTimePromtResponse - startTimeFirstWait
                console.log("STEP 3: time to get a response after first wait time (/nextblock has a message in queue):", diffWaitTimeResponseTime, '[ms]')
                ...
                let endTimeSpeechResponse = new Date().getTime();
                let diffSpeechResponseTime = endTimeSpeechResponse-startTime
                console.log("STEP 4: time to get a speech response from start time (/nextblock):", diffSpeechResponseTime, '[ms]')
